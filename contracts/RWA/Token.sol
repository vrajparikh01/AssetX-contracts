...
contract Token is IToken, AgentRoleUpgradeable, TokenStorage {
    ...
    modifier onlySTOFactory() {
        require(msg.sender == _STOFactory, "only STOFactory");
        _;
    }
    ...
    function setSTOFactory(address _STOFactoryAddress) external onlyOwner {
        _STOFactory = _STOFactoryAddress;
    }
    ...
    function transfer(address _to, uint256 _amount) public override whenNotPaused returns (bool) {
        ...
        require(_tokenIdentityRegistry.isVerified(_to), "receiver identity not verified");
    ...
    function isComplianceCheckPassed(
        address from,
        address to,
        uint256 amount
    ) external view onlySTOFactory() returns (bool) {
        return _tokenCompliance.canTransfer(from, to, amount);
    }
}
