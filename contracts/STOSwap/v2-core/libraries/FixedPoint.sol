...
library FixedPoint {
    ...
    function muluq(uq112x112 memory self, uq112x112 memory other) internal pure returns (uq112x112 memory) {
        ...
        uint112 lower_self = uint112(self._x & LOWER_MASK); // * 2^(-1)12
        uint112 upper_other = uint112(other._x >> RESOLUTION); // * 2^0
        uint112 lower_other = uint112(other._x & LOWER_MASK); // * 2^(-1)12

        // partial products
        uint224 upper = uint224(upper_self) * upper_other; // * 2^0
        uint224 lower = uint224(lower_self) * lower_other; // * 2^-224
        uint224 uppers_lowero = uint224(upper_self) * lower_other; // * 2^(-1)12
        uint224 uppero_lowers = uint224(upper_other) * lower_self; // * 2^(-1)12
